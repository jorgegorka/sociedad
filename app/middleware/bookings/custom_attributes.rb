  ...
  class CustomAttributes
    ...
    def call
      ...
      custom_attributes.each do |custom_attribute|
        return unless custom_attribute.block_on_schedule
    ...
    private
      ...
      attr_reader :user, :date, :schedule_category_id
      ...
      def invalid_attribute?(custom_attribute)
        bookings_for_today.includes(:booking_custom_attributes)
          .where(booking_custom_attributes: { custom_attribute_id: custom_attribute.id }).where.not(user_id: user.id).present?
      end
  end
end
